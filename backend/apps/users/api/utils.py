...
def generate_jwt_token(payload):
    ...
    access_payload["exp"] = datetime.datetime.now(
        datetime.timezone.utc
    ) + datetime.timedelta(minutes=int(settings.ACCESS_TOKEN_EXPIRATION_MINUTES))
    # Convert datetime to Unix timestamps
    access_payload["exp"] = int(access_payload["exp"].timestamp())

    access_token = jwt.encode(
        payload, settings.JWT_ACCESS_TOKEN_SECRET, algorithm=settings.JWT_ALGORITHM
    )
    ...
    refresh_payload = {
        ...
        + datetime.timedelta(days=int(settings.REFRESH_TOKEN_EXPIRATION_DAYS)),
        "iat": payload["iat"],
    }
    # Convert datetime to Unix timestamps
